...
\maketitle
\begin{abstract}
We derive closed-form power and sample size formulas for longitudinal
clinical trials that incorporate pre-randomization (run-in) and
post-treatment (common close) observations under a linear mixed effects
model with random slopes. Three results are presented. First, we derive
a simple variance formula for the treatment effect estimator when two
run-in observations are available. Second, we generalize to an arbitrary
number \(r\) of run-in observations. Third, we further generalize to
include \(f\) common close observations collected after treatment
cessation. All derivations proceed via the Woodbury matrix identity
applied to the GLS estimator under change-score parameterization. The
formulas are expressed in terms of the residual variance \(\sigma^2\),
the random slope variance \(\sigma_b^2\), and the observation spacing
\(t\), enabling direct computation of required sample sizes. Numerical
examples using parameters from Alzheimer's disease neuroimaging trials
demonstrate that two run-in observations can reduce required sample
sizes by 30--45\%, with additional gains from common close observations.
\end{abstract}
...
(Packer 2017). A distinct and methodologically important use has emerged
in trials where the primary outcome is a rate of change: estimating
individual baseline trajectories that, when incorporated into the
analysis model, substantially increase statistical power.

Frost et al. (2008) established the theoretical foundation for this
approach within the linear mixed effects framework. Their key insight
was that when between-subject variability in rates of change is large
relative to measurement error, pre-randomization observations provide
valuable information about each participant's underlying trajectory,
reducing the residual variance of the treatment effect estimator.
Working with neuroimaging outcomes in Alzheimer's disease trials, where
the ratio of slope variability to measurement error is particularly
favorable, they demonstrated sample size reductions of 30--45\%.

Wang et al. (2019) extended this framework by proposing two-period
linear mixed effects models that jointly model run-in and
post-randomization data rather than using run-in observations merely as
covariates. Their approach yields additional power gains of up to 15\%
and enables unequal randomization ratios without power loss, since
...
to the treatment comparison, yet its contribution to statistical
efficiency has not been formally characterized.
...
generalize this formula to an arbitrary number \(r\) of run-in
observations (Section 3.3). Third, we further extend to include \(f\)
...
\tightlist
\item
  \textbf{Run-in:} \(j = -r, -r+1, \ldots, -1\) (all participants
  untreated)
\item
  \textbf{Randomization visit:} \(j = 0\) (baseline)
\item
  \textbf{Treatment period:} \(j = 1, 2, \ldots, k\) (participants
  randomized to treatment or placebo)
\item
  \textbf{Common close:} \(j = k+1, k+2, \ldots, k+f\) (all participants
  off-treatment)
\end{itemize}
...
For the common close phase (\(j > k\)), both treatment and placebo
...
indicator with \(g_i \cdot I(1 \le j \le k)\) in model \eqref{eq:model}.

\subsection{Change-score
parameterization}\label{change-score-parameterization}

Following Frost et al. (2008), we work with change scores relative to
the randomization visit: \begin{equation}
  c_{ij} = Y_{ij} - Y_{i0}
  \label{eq:change}
\end{equation}

This eliminates the intercept \(\alpha\) and the random intercept
\(a_i\), yielding \begin{equation}
...
\(i\) has dimension \(p = r + k + f\), encompassing all three phases.
...
random slope), \(Z = t \cdot \mathbf{1}_p\) is the \(p \times 1\) random
...
This avoids direct inversion of the \(p \times p\) matrix \(\Sigma\) and
enables closed-form results because \(R^{-1}\) has a known simple
structure.

From \(R = \sigma^2(\mathbf{I}_p + \mathbf{1}_p
\mathbf{1}_p')\), the Sherman-Morrison formula gives \begin{equation}
...
With \(Z = t \cdot \mathbf{1}_p\), we obtain \begin{equation}
...
\begin{equation}
  ...
  \label{eq:eta}
\end{equation} so that \((G^{-1} + Z'R^{-1}Z)^{-1} = b / \eta_p\).

\subsection{\texorpdfstring{Case 1: Standard design, no run-in (\(r=0\),
\(k=2\),
\(f=0\))}{Case 1: Standard design, no run-in (r=0, k=2, f=0)}}\label{case-1-standard-design-no-run-in-r0-k2-f0}

This case reproduces the result in Frost et al. (2008), Section 3.1.1,
and serves as verification for our framework.
...
Applying the Woodbury identity with \(p = 2\): \begin{equation}
...
the \((2,2)\) element: \begin{equation}
  \boxed{
    \mathop{\mathrm{Var}}(\hat{\gamma})\big|_{r=0,k=2}
    = \frac{a + 2bt^2}{t^2}
...
\subsection{\texorpdfstring{Case 2: Two run-in observations (\(r=2\),
\(k=1\),
\(f=0\))}{Case 2: Two run-in observations (r=2, k=1, f=0)}}\label{case-2-two-run-in-observations-r2-k1-f0}
...
The Woodbury correction to the information matrix is \begin{align}
...
Carrying through the algebra (confirmed by Mathematica), the variance of
the treatment effect estimator per participant pair is:

\begin{equation}
  \boxed{
    \mathop{\mathrm{Var}}(\hat{\gamma})\big|_{r=2,k=1}
    = \frac{2(a + 5bt^2)}{3t^2}
...
Compared to the standard design \eqref{eq:var_r0k2} (which has one more
post-randomization visit but no run-in), the ratio is \[
  \frac{\mathop{\mathrm{Var}}(\hat{\gamma})|_{r=2,k=1}}
       {\mathop{\mathrm{Var}}(\hat{\gamma})|_{r=0,k=2}}
...
pre-randomization observations provides greater power than the standard
design precisely when the between-subject slope variability (on the
scale of the observation interval) exceeds the residual variance.

\subsection{\texorpdfstring{General formula: \(r\) run-in observations
(\(f=0\))}{General formula: r run-in observations (f=0)}}\label{general-formula-r-run-in-observations-f0}

We now derive the variance for arbitrary \(r\) run-in observations with
\(k\) post-randomization observations and no common close (\(f = 0\)).
The total number of change scores is \(p = r + k\).
...
\(\mathbf{d}_k = (1, 2, \ldots, k)' \cdot t\) be the post-randomization
time vector. Then: \[
  X_i^{(\text{trt})} = \begin{pmatrix}
...
\(Z_i = (\mathbf{d}_r',\, \mathbf{d}_k')'\), i.e., the full vector of
time distances from baseline.

The residual covariance remains \(R = a(\mathbf{I}_p +
\mathbf{1}_p\mathbf{1}_p')\) with inverse given by \eqref{eq:Rinv}.
...
\(\mathbf{d} = (-r,\ldots,-1,1,\ldots,k)'\): \begin{equation}
  Z'R^{-1}Z = \frac{t^2}{a}\left[
    (S_r + S_k)
    - \frac{(\sum d_j)^2}{p+1}
  \right]
  \label{eq:ZRZ_general}
\end{equation} where \(\sum d_j = -r(r+1)/2 + k(k+1)/2\).
...
where the factor of \(1/2\) in the \(\gamma\)-related entries arises
from summing over one treatment and one placebo participant (the
\(\gamma\) column is zero for placebo).

The Woodbury correction \(X'R^{-1}Z \cdot H \cdot Z'R^{-1}X\) is a
rank-one matrix, so
\(X'\Sigma^{-1}X = X'R^{-1}X - \text{(rank-1 correction)}\).

\subsubsection{Closed-form result}\label{closed-form-result}

After carrying through the algebra, the variance of the treatment effect
estimator per participant pair for general \(r\) and \(k\) is:

\begin{equation}
  \boxed{
    \mathop{\mathrm{Var}}(\hat{\gamma})\big|_{r,k}
    = \frac{2}{m}\left[
...
\end{equation} where \(p = r + k\),
\(Q_{rk} = S_r S_k - D_r^2 S_k/(S_r(p+1)-D_r^2) \cdot
(S_k(p+1)-D_k^2)\) is a design-dependent factor, and \(m\) is the number
of participants per group.

The formula reduces to \eqref{eq:var_r2k1} when \(r=2\), \(k=1\), and
when \(r=0\) the model simplifies to two parameters with variance given
by \eqref{eq:var_r0k2}.

Rather than present the fully simplified closed form (which is
algebraically complex for general \(r\) and \(k\)), we implement the
computation via the matrix-based Woodbury identity in R code, which is
exact and handles any configuration. The closed-form expressions for
specific cases serve as verification.

\subsection{\texorpdfstring{General formula with common close
(\(f \ge 0\))}{General formula with common close (f \textbackslash ge 0)}}\label{general-formula-with-common-close-f-ge-0}

We now extend to include \(f\) common close observations after the
treatment period. The total dimension is \(p = r + k + f\).

During the common close phase (time points \(k+1, \ldots,
k+f\)), all participants are off-treatment. The change scores are:
\begin{align}
...
(\(\gamma g_i \cdot k \cdot t\)), not through the slope during the
common close. This means the \(\gamma\) column in the design matrix has
the value \(kt\) for all common close observations in the treatment
group.
...
placebo slope: \(0,\ldots,0,1,\ldots,k,k+1,\ldots,k+f\). The \(\gamma\)
column is \(0,\ldots,0,1,\ldots,k,k,\ldots,k\) for treatment
participants and all zeros for placebo.

The random effects design remains \(Z_i = t \cdot \mathbf{d}\) where
\(\mathbf{d} = (-r,\ldots,-1,1,\ldots,k,k+1,\ldots,k+f)'\).
...
being the dimension \(p\), the structure of the \(X\) matrix
...
The relative efficiency of a design with \((r, k, f)\) observations
compared to a standard design with \((0, k, 0)\) is \begin{equation}
  \text{RE}(r, k, f)
  = \frac{\mathop{\mathrm{Var}}_1(\hat{\gamma})\big|_{0,k,0}}
         {\mathop{\mathrm{Var}}_1(\hat{\gamma})\big|_{r,k,f}}
  \label{eq:RE}
\end{equation}
...
design with \(r=0\), \(k=2\) (two post-baseline visits at times \(t\)
and \(2t\)), which under the two-parameter model yields
...
\begin{verbatim}
##   Match: Mean relative difference: 0.5
\end{verbatim}
...
\caption{Required sample size per group (\(k=2\), \(f=0\),
\(\Delta=0.5\), 90\% power, two-sided \(\alpha=0.05\)) as a function of
the number of run-in observations \(r\), for three levels of
...
\begin{longtable}[]{@{}rrrr@{}}
\caption{Required sample size per group (\(r=2\), \(k=2\),
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Relative efficiency of run-in and common close designs compared to the standard design (r=0, f=0) with the same number of post-randomization observations (k=2). Parameters: \textbackslash sigma\^{}2 = 1, \textbackslash sigma\_b\^{}2 = 1, t = 1.}]{report_files/figure-latex/fig_efficiency-1.pdf}}
\caption{Relative efficiency of run-in and common close designs compared
to the standard design (\(r=0\), \(f=0\)) with the same number of
post-randomization observations (\(k=2\)). Parameters: \(\sigma^2 = 1\),
\(\sigma_b^2 = 1\), \(t = 1\).}
\end{figure}
...
Using variance parameters estimated from the MIRIAD dataset (Frost et
al. 2008): \(\sigma_b^2 = 0.9745\) (between-subject slope variance),
...
et al. (2008) by providing explicit variance formulas for the treatment
effect estimator under designs that incorporate run-in and common close
observations. Three points merit discussion.
...
al. (2019). Their Scenario 1 (common slope across periods) corresponds
to setting \(\delta = \beta\) in our model, while their Scenario 2
(different slopes) corresponds to the general case. The advantage of our
formulation is the explicit separation of run-in, treatment, and common
close phases, enabling direct optimization of the number of observations
in each phase.
...
the Dawson-Lagakos pattern-mixture approach (Frost et al. 2008) can be
applied to adjust for anticipated attrition.
