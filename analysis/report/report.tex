...
\frac{1}{p+1}\mathbf{1}\mathbf{1}')\), the information matrix summed
...
post-randomization time sums have opposite signs (\(\sum d_r < 0\),
\(\sum d_k > 0\)).
...
The variance \(\mathop{\mathrm{Var}}(\hat{\gamma})\) for general \(r\)
and \(k\) does not admit a simple closed-form expression. The formula
depends on \(\sigma^2\) and \(\sigma_b^2\) in a ratio involving \(S_r\),
\(S_k\), \(D_r\), \(D_k\), and \(p\), and the interaction between the
Woodbury correction and the information matrix produces algebraically
complex expressions.
...
configuration \((r, k, f)\). The closed-form expressions for \(r=0\)
(\ref{eq:var_r0k2}) and \(r=2\) (\ref{eq:var_r2k1}) serve as
