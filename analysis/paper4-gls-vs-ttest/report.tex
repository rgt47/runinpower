...
\maketitle
\begin{abstract}
Three approaches exist for incorporating run-in observations into the
analysis of longitudinal clinical trials. The GLS estimator under the
linear mixed model uses the full covariance structure to extract maximum
information from all observations. The ANCOVA estimator regresses
post-randomization means on the run-in mean as a covariate, requiring no
specification of the random effects covariance. The averaged
change-score estimator computes a simple difference of phase means and
applies a \(t\)-test, requiring no variance component estimation at all.
We derive the relative efficiency of these three approaches as a
function of the trial design \((J_0, J_1, J_2)\) and the variance
parameters \((\sigma^2, \sigma_b^2)\). Under correct model
specification, GLS \(\le\) ANCOVA \(\le\) averaged in variance. Under
misspecification of \(G\) or \(R\), the simpler estimators can
outperform GLS. We provide conditions under which ANCOVA loses less than
5\% efficiency relative to GLS and investigate robustness via
simulation.
\end{abstract}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Effect of analyzing under compound symmetry when the true correlation is AR(1). Sandwich variance of GLS (solid) vs averaged change-score (dashed) as a function of \textbackslash rho.}]{report_files/report_files/figure-latex/misspec_R-1.pdf}}
