...
\maketitle
\begin{abstract}
Given a fixed total number of observations \(J = J_0 + J_1 + J_2\) and a
fixed trial duration, how should visits be allocated across run-in
(\(J_0\)), treatment (\(J_1\)), and common close (\(J_2\)) phases to
minimize the variance of the treatment effect estimator? We formulate
this as a discrete optimization problem over the Woodbury-based variance
formula from Thomas (2026) and characterize the solution as a function
of the signal-to-noise ratio \(\sigma_b / \sigma\). We show that the
optimal allocation depends primarily on this ratio and that the common
close phase is beneficial only when the ratio exceeds a threshold that
depends on \(J_1\). We further extend to non-equally-spaced designs,
where the timing of observations within each phase is also optimized. We
also consider replicated endpoint observations: closely spaced
measurements at the end of the treatment period that average out
measurement error on the final assessment, analogous to the run-in
averaging mechanism but applied at the post-treatment end. The
pattern-mixture approach of Frost et al. (2008) is incorporated to
account for dropout.
\end{abstract}
